...
def J(w, faces, labels, alpha = 0.):
    ...
    return 0.5 * np.dot(inner, inner.T)


def J_regularized(w, faces, labels, alpha = 0.):
    return 0.0


def gradJ(w, faces, labels, alpha = 0.):
    """ gradJ = 0.5 * 2 * X.T(Xw - y) = X.T(Xw-y) """
    inner = np.dot(faces, w) - labels
    return np.dot(faces.T, inner)
...
def gradientDescent (trainingFaces, trainingLabels, testingFaces, testingLabels, alpha = 0.):
    ...
    epsilon = 0.000001
    delta = 0.001
    prevJ = J(w, trainingFaces, trainingLabels)
    diff = 1000
    count = 0
    print(diff, delta)
    while diff > delta:
        update = epsilon * gradJ(w, trainingFaces, trainingLabels)
        w = w - update
        newJ = J(w, trainingFaces, trainingLabels)
        diff = prevJ - newJ
        prevJ = newJ
        count += 1
        if count % 100 == 0: print(newJ, diff)
    return w
...
#     vc = cv2.VideoCapture()
#     vc.open(0)
#     faceDetector = cv2.CascadeClassifier("haarcascade_frontalface_alt2.xml") # TODO update path
#     while vc.grab():
#         (tf,im) = vc.read()
